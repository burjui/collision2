...
impl<'a> BindGroupEntriesStructBuilder<'a> {
    ...
    fn get_resource_type_from_binding(&self, binding_type: &naga::TypeInner) -> BindResourceType {
    ...
    fn create_entry_from_parameter(&self, binding_var_name: &Ident, binding: &SingleBindGroupEntry) -> TokenStream {
        let entry_cons = self.generator.entry_constructor;
        let binding_index = binding.binding_index as usize;
        let demangled_name = RustSourceItemPath::from_mangled(binding.name.as_ref().unwrap(), self.containing_module);
    ...
    fn assign_entries_from_parameters(&self, param_var_name: Ident) -> Vec<TokenStream> {
        self.data
            ...
            .map(|binding| {
                let demangled_name =
                    RustSourceItemPath::from_mangled(binding.name.as_ref().unwrap(), self.containing_module);
                let binding_name = Ident::new(&demangled_name.name, Span::call_site());
    ...
    fn binding_field_tuple(&self, binding: &SingleBindGroupEntry) -> (TokenStream, TokenStream) {
        let rust_item_path = RustSourceItemPath::from_mangled(binding.name.as_ref().unwrap(), self.containing_module);
    ...
    pub(super) fn build(&self) -> TokenStream {
        let (entries_param_fields, entries_fields): (Vec<_>, Vec<_>) =
            self.data.bindings.iter().map(|binding| self.binding_field_tuple(binding)).collect();

        let entry_collection_name = self.generator.bind_group_entries_struct_name_ident(self.group_no);
        let entry_collection_param_name =
            format_ident!("{}Params", self.generator.bind_group_entries_struct_name_ident(self.group_no));
...
impl<'a> BindGroupStructBuilder<'a> {
    fn bind_group_layout_descriptor(&self) -> TokenStream {
        let entries: Vec<_> = self.data.bindings.iter().map(|binding| &binding.layout_entry_token_stream).collect();

        let bind_group_label = format!("{}::BindGroup{}::LayoutDescriptor", self.sanitized_entry_name, self.group_no);
    ...
    fn struct_name(&self) -> syn::Ident {
        self.options.wgpu_binding_generator.bind_group_layout.bind_group_name_ident(self.group_no)
    }
    ...
    fn bind_group_struct_impl(&self) -> TokenStream {
        ...
        let bind_group_entries_struct_name =
            self.options.wgpu_binding_generator.bind_group_layout.bind_group_entries_struct_name_ident(self.group_no);
...
impl<'a> SingleBindGroupBuilder<'a> {
    pub(super) fn build(&self) -> RustSourceItem {
        ...
        let additional_layout = if let Some(additional_generator) = &self.options.extra_binding_generator {
        ...
        let source_item_path =
            RustSourceItemPath::new(self.containing_module.into(), bindgroup_struct_builder.struct_name().to_smolstr());
...
fn check_texture_filterability(
    ...
    let fully_qualified_name = demangle_and_fully_qualify_str(name, Some(invoking_entry_module));
...
fn generate_binding_type_for_type(
    ...
    match &binding_type.inner {
        ...
        naga::TypeInner::Image { dim, class, arrayed } => {
            let view_dim = if *arrayed {
                match dim {
                    ...
                    naga::ImageDimension::D1 | naga::ImageDimension::D3 => {
                        panic!("1 Dimensional and 3 Dimensional textures have an array view dimension")
                    }
                }
            } else {
                match dim {
                ...
                naga::ImageClass::Sampled { kind, multi } => {
                    let sample_type = match kind {
                        ...
                        naga::ScalarKind::Float => {
                            let filterable = check_texture_filterability(binding_name, invoking_entry_module, options);
...
fn bind_group_layout_entry(
    ...
    let wgpu_binding_type =
        generate_binding_type_for_type(binding_type, invoking_entry_module, naga_module, options, address_space, &name);

    let doc =
        format!(" @binding({}): \"{}\"", binding_index, demangle_and_fully_qualify_str(name.as_ref().unwrap(), None),);
...
impl<'a> SingleBindGroupData<'a> {
    ...
    pub fn are_all_same_module(&self) -> bool {
        ...
        self.bindings.iter().all(|b| b.item_path.module == first_module)
    }
...
impl<'a> SingleBindGroupEntry<'a> {
    pub fn new(
        ...
        let item_path = RustSourceItemPath::from_mangled(name.as_ref().unwrap(), invoking_entry_module);
