...
<ul>
<li>Grid-based broad phase assumes particles of similar sizes. Performance degrades quickly if they differ significantly. There was a BVH-based broad phase, but I removed it for now. Look up the repo history if you want to check it out.
<li>The code barely has any comments, because it requires some proficiency in GPU programming and the lion's share of it is self-explanatory if you have it. And I'm lazy.
<li>Simulation step is the DT constant in `main.rs`
<li>Indirect dispatch is utilised where appropriate to avoid CPU<->GPU data transfer.
<li>Not published on crates.io since it's not a tool
<li>Only tested on Manjaro Linux with RADV Vulkan driver on RX 560.
<li>Definitely has bugs. If I had a dollar for each one I've found over the course of development, I could easily fund my beer-brewing hobby 😆
</details>
